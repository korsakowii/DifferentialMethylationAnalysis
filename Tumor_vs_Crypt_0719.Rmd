...
data <- data.frame(x=fit[,1],y=fit[,2],
                   pheno = pheno,
                   Sample_Name = id)
...
p <- plot_ly(data = data, x = ~x, y = ~y, 
             ...
             add_annotations(
               ...
               showarrow = FALSE,
               font = list(size = 15))
p <- layout(p, title = list('MDS 1000 most variable positions'), 
            margin = m, showlegend = F, 
            xaxis = list(title = 'Principal Component 1',
                      zeroline = TRUE,
                      range = c(-200, 200)),
            yaxis = list(title = 'Principal Component 2',
...
p <- layout(p, 
            title = paste("Top ", length(rank),
                          " Genes Mostly Influenced by DMPs",
                          sep = ""),
            barmode = "stack", 
...
```{r Geneplot}
# select gene
mycol <- brewer.pal(9, "Paired")[c(1, 5, 2, 6, 3, 7)]
pvalueCutoff <- 0.05
abslogFCCutoff <- 1
geneName <- "ZNF331"
beta <- Badj
...
p <- X %>% 
  ...
  add_markers(x = ~jitter(pos), y = ~Value, color = ~pheno, colors = mycol,
              marker = list(opacity = 0.9, size = 3),
              hoverinfo = "text",
